...
function config.write(self, section, value)
	...
	nixio.fs.writefile("/etc/dnsmasq_route.ini", value)
        luci.sys.call("/etc/init.d/dnsmasq_route restart >/dev/null;/etc/init.d/dnsmasq restart >/dev/null")
end
...
function enable.write(self, section, value)
        Flag.write(self, section, value)
        if value == "1" then
                luci.sys.call("/etc/init.d/dnsmasq_route enable >/dev/null;uci set dhcp.cfg01411c.logqueries='1' >/dev/null;uci commit >/dev/null;/etc/init.d/dnsmasq_route restart >/dev/null")
        else
                luci.sys.call("/etc/init.d/dnsmasq_route stop >/dev/null;/etc/init.d/dnsmasq_route disable >/dev/null;uci del dhcp.cfg01411c.logqueries >/dev/null;uci commit >/dev/null;/etc/init.d/dnsmasq reload >/dev/null")
        end
end
...
function dnslog.cfgvalue(self, section)
	local t = io.popen('logread -l 10000 -e dnsmasq |dnsmasq_log -V -l 600 |sort -r |head -n 200')
	return t:read("*all")
end
