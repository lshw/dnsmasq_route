...
function config.write(self, section, value)
	...
	nixio.fs.writefile("/etc/dnsmasq_route.ini", value)
        luci.sys.call("/etc/init.d/dnsmasq_route restart;/etc/init.d/dnsmasq restart >/dev/null")
end

function enable.write(self, section, value)
        if value == "1" then
                luci.sys.call("/etc/init.d/dnsmasq_route enable;/etc/init.d/dnsmasq_route start;uci set dhcp.cfg01411c.logqueries='1';uci commit;/etc/init.d/dnsmasq restart >/dev/null")
        else
                luci.sys.call("/etc/init.d/dnsmasq_route stop;/etc/init.d/dnsmasq_route disable;uci del dhcp.cfg01411c.logqueries;uci commit;/etc/init.d/dnsmasq reload >/dev/null")
        end
