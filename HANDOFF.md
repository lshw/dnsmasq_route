...
这个仓库是一个 OpenWrt `dnsmasq_route` 插件，作用是：

- 根据 `/etc/dnsmasq_route.ini` 里的域名列表，把这些域名交给指定 DNS 服务器解析。
- 把解析出来的 IPv4 地址加入 `ipset` 或 `nftset`。
...
代码层面目前**没有卡住的未完成修改**，工作区是干净的。

真正还没做的是：

- 没有在真实 OpenWrt 设备上回归验证 LuCI 页面修改后的运行效果
- 没有现场确认 `localnet_only=0` 时，最终生成的 nft/iptables 规则是否确实去掉源地址限制
- 没有做打包安装测试
...
2. 每次修改后检查服务是否真的重启并重新生效：
   ...
   - 或 `iptables -t mangle -S DNSMASQ_ROUTE`
